...
import {logoutUser} from "../../service/auth-service.ts";
...
export const Header = () => {
    ...
    function onHandleLogout() {
        logoutUser();
        userDispatcher({type:'sign-in', user: null});
        navigate('/app');
    }
                ...
                <div className="relative sign-in flex justify-end items-center gap-3">
                    <div className={`${user ? 'flex' : 'hidden'} user-image`}>
                        <Dropdown
                            ...
                            <Dropdown.Divider/>
                            <Dropdown.Item onClick={onHandleLogout}>Sign out</Dropdown.Item>
                        </Dropdown>
                    </div>
                    <button
                        className={`${user ? 'hidden' : 'block'} relative inline-flex items-center justify-center p-0.5 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-purple-600 to-blue-500 group-hover:from-purple-600 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800"`}
